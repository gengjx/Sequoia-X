...
_DEFAULT_STRATEGY_QUALITY: dict[str, int] = {
    "multi_factor": 80,   # S级 核心：5.3年年化+22.6%，超额+6.4%，唯一穿越牛熊
    "bottom": 66,         # A级 年化+11.9%，超额-4.3%（辅助）
    "flag": 59,           # B级 年化+7.0%（辅助）
    "pullback": 49,       # C级 年化+1.9%（已降权）
    "volume_extreme": 46, # C级 年化+1.2%（已降权）
    "ma_volume": 42,      # C级 年化-1.4%（已降权）
    "shakeout": 21,       # D级 年化-20.8%，回撤-81%（已废弃）
    "turtle": 7,          # D级 年化-17.1%，回撤-72%（已废弃）
    "limit_down": 4,      # D级 年化-33.2%，回撤-90%（已废弃）
    "dragon": 1,          # D级 年化-27.4%，回撤-87%（已废弃）
    "rps": 1,             # D级 年化-29.3%，回撤-91%（已废弃）
    "auction": 85,        # 集合竞价A级（实时资金意图，高开+放量+量比大）
    "lhb_follow": 60,     # 龙虎榜跟买（事件驱动，与因子策略低相关）
    "sector_rotation": 55, # 板块轮动（中期趋势跟随，板块维度独立价值）
}
