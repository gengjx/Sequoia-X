...
### 大盘分析（Market Analysis）

- 一键生成每日盘面分析报告，导航栏点击「大盘分析」进入 `/market`
- 报告包含七大板块：
    1. **盘面总览**：涨跌家数、涨跌停、两市成交额、盘面信号评分（0-100，加权宽度45% + 平均涨跌幅30% + 涨跌停比25%）
    2. **指数结构**：上证、深证、创业板、上证50、沪深300 五大指数行情表及 20 日支撑/压力位
    3. **板块主线**：证监会行业分类涨跌幅排行（领涨 / 领跌 Top 5）
    4. **资金与情绪**：成交活跃度、涨跌停比、市场宽度与赚钱效应分析
    5. **消息催化**：基于领涨板块的主题提示
    6. **明日交易计划**：进攻 / 均衡 / 防守建议及对应仓位、关注与回避方向
    7. **风险提示**：指数分化、涨幅过大、资金过度集中等结构性风险
- 「生成报告」按钮异步执行（约 5~10 秒聚合本地行情与 baostock 指数数据），完成后自动渲染
- 「刷新行业」按钮从 baostock 重新拉取行业分类并缓存至 `stock_industry` 表（约 30~45 秒）
- A 股配色惯例：红色代表上涨（涨），绿色代表下跌（跌）
